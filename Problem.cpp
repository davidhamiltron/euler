...
Problem::Problem(int number, std::string &description, std::function<std::string(void)> solver) :
...
Problem4::Problem4()
{
	m_szDescription = "Find the largest palindrome made from the product of two 3-digit numbers.";
}
...
bool Problem5::Solve()
{
	...
	int iDivisibleByAllLessThan = 20;

	int iSmallestNumber = iDivisibleByAllLessThan;
	...
	while(!bDivisible)
	{
		...
		for(int i = 1; i <= (iDivisibleByAllLessThan - 1); i++)		// -1 because we are only examining multiples of the highest factor
		{
			if(iSmallestNumber%i != 0)
			{
				bDivisible = false;
				break;
			}
		...
		iSmallestNumber += iDivisibleByAllLessThan;
	}

	m_szAnswer = ToString(iSmallestNumber);
	return true;
}
...
Problem6::Problem6()
{
	m_szDescription = "What is the difference between the sum of the squares and the square of the sums?";
}
...
bool Problem6::Solve()
{
	...
	iSquareOfSum *= iSquareOfSum;

	iDifference = iSquareOfSum - iSumOfSquares;

	m_szAnswer = ToString(iDifference);
	return true;
}
...
Problem7::Problem7()
{
	m_szDescription = "Find the 10001st prime.";
}

__int64 GetPrime(int iIndex)
{

	int iCurrentIndex = 1;	// first prime is 2
	int iNum = 3;
